...
const favoritarFilme = async (tokenID) => {
    const results = await fetch(`https://api.themoviedb.org/3/account/{account_id}/favorite?api_key=${apiKey}&session_id=${tokenID}`,{
        method: 'POST',
        headers: {
...
const pesquisaFilme = async (filme) =>{
    const resposta = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&language=pt-BR&query=${filme}&page=1&include_adult=false`)
    if (resposta.ok) {
        return resposta.json()
...
const pesquisaFilmeID = async (id) =>{
    const resposta = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${apiKey}4&language=pt-BR`)
    if (resposta.ok) {
        return resposta.json()
